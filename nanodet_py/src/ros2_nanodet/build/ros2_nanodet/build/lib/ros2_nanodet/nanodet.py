#!/usr/bin/env python3
##实现将nanodet python版本转换为ROS noetic
...
class my_nanodet():
    def __init__(self, input_shape=320, prob_threshold=0.4, iou_threshold=0.3, model_path=os.path.join(os.path.dirname(__file__), 'nanodet.onnx'), clsname_path=os.path.join(os.path.dirname(__file__), 'coco.names')):
...
def main():
    ...
    if srcimg is None:
        print(f"Error: Unable to load image at {args.imgpath}")
        return
